...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Acquire, clean, and create features from heart attack data"
    )
    parser.add_argument(
    ...
    # Acquire data from the s3 and save to disk
    ad.acquire_data(run_config["data_source"], artifacts / "heart.data")
    print(f"Artifacts are being saved in: {artifacts}")

    # Create structured dataset from raw data; save to disk
    data = cd.create_dataset(artifacts / "heart.data", config["create_dataset"])
    cd.save_dataset(data, artifacts / "heart.csv")
